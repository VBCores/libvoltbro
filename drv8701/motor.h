...
// Following functions do simple calculations that compiler probably optimizes
...
radian_speed_to_pwm(DriverConfig* config, double radian_speed) {
    ...
    if (config->is_small) {
        ...
        // from 0-1 to 0.5-1 It DOES work with 0-1 but PID signal is a lot
        // smoother like this (again, for small motors)
