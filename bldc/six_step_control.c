...
force_inline void observer(DriverState* driver, const IncrementalEncoder* encoder, float dt) {
...
void six_step_control(
    ...
    if ( passed_time_abs > driver->sampling_interval ) {
        ...
        int32_t aligned_encoder_data = abs_value - as5048a_config.common.elec_offset;
        ...
        if ( abs(prev_abs_value - aligned_encoder_data) > half_cpr ) {
            ...
            driver->ShaftAngle = pi2 * ((float) filtered_data / (float) as5048a_config.common.CPR);

            static float prev_velocity = 0;
            const float travel = pi2 * ((float) abs_diff / (float) as5048a_config.common.CPR);
    ...
    float v_a = 0; float v_b = 0; float v_c = 0;
