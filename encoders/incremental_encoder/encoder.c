...
force_inline EncoderStep get_encoder_step(IncrementalEncoder* encoder) {
    if (encoder->state_3) {
        if (encoder->state_1) return 1;
        if (encoder->state_2) return 5;
        return 0;
    }
    if (encoder->state_1) {
        if (encoder->state_2) return 3;
        return 2;
    }
    return 4;
...
IncrementalEncoder* make_incr_encoder(
    ...
    make_incr_encoder_reserved(encoder, CPR, inverted, pin_gpiox, pin_1, pin_2, pin_3);
...
void handle_encoder_channel(IncrementalEncoder* encoder, pin channel) {
    ...
    if ( (activated_pin == encoder->last_activated + 1) || (encoder->last_activated == 2 && activated_pin == 0) ) {
